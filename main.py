...
async def _stream_claude(req: ChatRequest):
    ...
    async with http().stream("POST", f"{DATA999_BASE}/v1/messages", headers=headers, json=body) as r:
...
async def _stream_openai_from(req: ChatRequest, base: str, key: str):
    ...
    got_text = False
    async with http().stream("POST", f"{base}/v1/chat/completions", headers=headers, json=body) as r:
